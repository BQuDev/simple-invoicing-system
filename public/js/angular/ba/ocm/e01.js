...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'How and Why do middle managers support and resist strategic change?Introduce strategic change and discuss ways of middle managers to support strategic change, Reasons for middle managers to support strategic change, ways of middle managers resist strategic change and reasons for middle managers to resist strategic change.';
        }, headerTooltip: 
        function( col ) {
          return 'How and Why do middle managers support and resist strategic change?Introduce strategic change and discuss ways of middle managers to support strategic change, Reasons for middle managers to support strategic change, ways of middle managers resist strategic change and reasons for middle managers to resist strategic change.';
        }
    ...
    { name: 'c2', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 2' , cellTooltip: 
        function( row, col ) {
          return 'Why do model of planned change not bring about cultural change?Define and explain different change models used for change management. Discuss the reasons for the referred models are not addressing culture changes in organisations based on relevant literature sources.';
        }, headerTooltip: 
        function( col ) {
		  return 'Why do model of planned change not bring about cultural change?Define and explain different change models used for change management. Discuss the reasons for the referred models are not addressing culture changes in organisations based on relevant literature sources.';
        }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'How do leadership behaviours positively and negatively impact upon employee commitment to organisational change?Explain different types of leadership behaviours and its impact on organization change in terms of increasing or decreasing employee commitment resulting in negative or positive impacts';
        }, headerTooltip: 
        function( col ) {
		  return 'How do leadership behaviours positively and negatively impact upon employee commitment to organisational change?Explain different types of leadership behaviours and its impact on organization change in terms of increasing or decreasing employee commitment resulting in negative or positive impacts';
        }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
          ...
          $.ajax({
                  type        : 'POST',
                  url         : 'save_marks_for_OCM_A_01',
                  data        : formData,
                  dataType    : 'json',
                  success     : function(data) {
					  if((data == 1)&&(document.getElementById('auto_refresh').checked)){
						  $http.get('students_for_marks_OCM_A_01')
            .success(function(data) {
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_OCM_A_01_excel_export", '_blank');
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_OCM_A_01_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
