...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'Presentation structure and overall affect';
        }, headerTooltip: 
        function( col ) {
          return 'Presentation structure and overall affect';
        }
    ...
    { name: 'c2', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 2' , cellTooltip: 
        function( row, col ) {
          return 'Presentation clarity and style';
        }, headerTooltip: 
        function( col ) {
		  return 'Presentation clarity and style';
        }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'Suitable set of objectives,Production of appropriate strategy for the company which will enable the company to achieve the objectives. The strategy must use suitable models, Budget (forecast cash flow and profit and loss budgets to be provided for the project) ';
        }, headerTooltip: 
        function( col ) {
		  return 'Suitable set of objectives,Production of appropriate strategy for the company which will enable the company to achieve the objectives. The strategy must use suitable models, Budget (forecast cash flow and profit and loss budgets to be provided for the project) ';
        }
    ...
    { name: 'c4', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 4' , cellTooltip: 
        function( row, col ) {
          return 'Understanding and explanation of content demonstrated';
        }, headerTooltip: 
        function( col ) {
		  return 'Understanding and explanation of content demonstrated';
        }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
          ...
          $.ajax({
                  type        : 'POST',
                  url         : 'save_marks_for_MC_A_02',
                  data        : formData,
                  dataType    : 'json',
                  success     : function(data) {
					  if((data == 1)&&(document.getElementById('auto_refresh').checked)){
						  $http.get('students_for_marks_MC_A_02')
            .success(function(data) {
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_MC_A_02_excel_export", '_blank');
    ...
    $scope.edit = function(san){
	 ...
	 /*
	  var formData1 = {
	        ...
	        $.ajax({
                  type        : 'GET',
                  url         : 'save_marks_for_MC_A_02_glanced_word',
                  data        : formData1,
                  dataType    : 'json',
                  success     : function(data) {
                  }
			...
			//  console.log("save_marks_for_IM_A_02_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type="+document.querySelector('input[name="export_type"]:checked').value);
			  //console.log("save_marks_for_IM_A_02_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type="+document.querySelector('input[name="export_type"]:checked').value);
			  //window.location = "save_marks_for_MC_A_02_glanced_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type="+document.querySelector('input[name="export_type"]:checked').value;
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_MC_A_02_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
