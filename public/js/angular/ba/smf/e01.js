...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'Introduction-aims or purpose of the report,very brief overview of the chosen corporation, final paragraph that outlines the next section of the report';
        }, headerTooltip: 
        function( col ) {
          return 'Introduction-aims or purpose of the report,very brief overview of the chosen corporation, final paragraph that outlines the next section of the report';
        }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'Analysis specify the material to be analysed, its source, and why it is relevant for this particular analytical approach.For each analysis, systematically apply the approach and explanation of the candidate’s findings.(Maximum 20 marks for each analysis)';
        }, headerTooltip: 
        function( col ) {
		  return 'Analysis specify the material to be analysed, its source, and why it is relevant for this particular analytical approach.For each analysis, systematically apply the approach and explanation of the candidate’s findings.(Maximum 20 marks for each analysis)';
        }
    ...
    { name: 'c4', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 4' , cellTooltip: 
        function( row, col ) {
          return 'Discussion and conclusion Discuss and compare and contrast the findings from the two analyses, including contradictory findings and what could account for them.Relate the findings and discussion to the aims and purpose of the report, and specify any conclusions the candidate has reached.';
        }, headerTooltip: 
        function( col ) {
		  return 'Discussion and conclusion Discuss and compare and contrast the findings from the two analyses, including contradictory findings and what could account for them.Relate the findings and discussion to the aims and purpose of the report, and specify any conclusions the candidate has reached.';
        }
    ...
    { name: 'c5', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 5' , cellTooltip: 
        function( row, col ) {
          return 'Presentation and references Meaningful titleUse the Harvard referencing system for citing sourcesFollowed guidance on report structureAppearance and written expression';
        }, headerTooltip: 
        function( col ) {
		  return 'Presentation and references Meaningful titleUse the Harvard referencing system for citing sourcesFollowed guidance on report structureAppearance and written expression';
       }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_SMF_A_01_excel_export", '_blank');
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_SMF_A_01_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
